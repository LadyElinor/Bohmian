...
def main():
    ...
    # Explicit in-policy closure bounds from A-001 and A-002
    ...
    out_dir = out_root / f"grqm_cycle4_inpolicy_confirm_{ts}"
    ...
    for omega in omega_list:
        for alpha in alpha_list:
            ...
            hs = []
            for cfg in policy_perturbations:
    ...
    with open(out_dir / "cycle4_inpolicy_confirm_summary.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader(); w.writerows(rows)

    with open(out_dir / "proxy_agreement_v4_inpolicy.csv", "w", newline="", encoding="utf-8") as f:
